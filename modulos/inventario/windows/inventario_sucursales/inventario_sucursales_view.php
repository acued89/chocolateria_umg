...
class inventario_sucursales_view extends inventario_view{
    static $_instance;
    private $objView;
    ...
    public function drawPage(){
        $this->get_javascript();
        $this->getButtons("ButtonsSucursales", $this->getArrBotones(),5);
        $this->drawContenido();        
    }
    public function get_javascript(){
        ...
        <script type="text/javascript">
            ...
            function grabar(){
                var objMarca = $("#frm_sucursales_name");
                    ...
                    $.get(link,function(data){
                        if(isnew){
                            if(data.NewResult === "true"){
                                myWidget.alertDialog("Sucursal agregada exitosamente");
                            }
                            else{
                                myWidget.alertDialog("La Sucursal ya ha sido registrada, intente ingresando una nueva Sucursal");
                            }
                            
                        }
                        else{
                            if(data.UpdateResult === "true"){
                                myWidget.alertDialog("La Sucursal fue actualizada");
                            }
                            else{
                ...
                if(objMarca.val() !== ""){
                    ...
                    $.get(link,function(data){
                        if(data.DeleteResult === "true"){
                            myWidget.alertDialog("Sucursal eliminada");
                        }
                        else{
                    ...
                    myWidget.alertDialog("Seleccione una Sucursal para poder eliminarla")
                }
...
//                            tr.append(td)
            ...
            function seleccionar(data){
                $("#frm_sucursales_name").val(data.Name);
                $("#frm_sucursales_name").data({
                    "isNew":0,
                    "id" : data.IdTradeMark
                });
            }
    ...
    public function getArrBotones(){
        ...
        $this->arrBotons[3] = array("title" => "Listado", "name" => "btnBuscar", "onclick" => "buscar()");
    ...
    public function drawContenido(){
        $this->initForm("frm_sucursales");
        ?>
        <div id="content-sucursales">
                ...
                <div class="panel-body">
                    <div class="col-lg-6">
                        <div class="form-group">
                            <label>Nombre</label>
                            <input class="form-control" size="30" name="frm_sucursales_name" id="frm_sucursales_name">
                        <label>Dirección</label>
                            <input class="form-control" size="30" name="frm_sucursales_direccion" id="frm_sucursales_direccion">
                        </div>
                    </div>                                        
                </div>
            </div>
