...
class inventario_marcas_controller extends inventario_controller{
    ...
    public function getOperacion(){
        ...
        */
        if($op == "new"){
            $link = "http://umgsk8ertux.azurewebsites.net/Services/Products.svc/New/{$data}";
            $exec = RestClient::get($link); 
        }
        else if($op =="update"){
            $link = "http://umgsk8ertux.azurewebsites.net/Services/Products.svc/Update/{$id}/{$data}";
            $exec = RestClient::get($link); 
        }
        elseif($op == "delete"){
            $link = "http://umgsk8ertux.azurewebsites.net/Services/Products.svc/Delete/{$id}";
            $exec = RestClient::get($link);
        }
        else{
